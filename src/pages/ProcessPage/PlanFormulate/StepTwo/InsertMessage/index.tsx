...
export default React.memo((params:{id:number}) => {
  const { id } = params;
  ...
  const getRequest = async () => {
    // Todo:页码未添加
    const storeInfo = await getDataInfo('/item/info', { id }).then(res => { return res.data.list })
    ...
    <>
      ...
      <Modal title="Basic Modal" visible={isModalVisible} onOk={handleOk} onCancel={handleCancel}>
        <Form {...layout} name="nest-messages" form = {form} onFinish={onFinish} validateMessages={validateMessages}>
          ...
          <Form.Item name={'amount'} label="养殖池选择" >
            <InputNumber disabled={inputState}/>
          </Form.Item>
          <Form.Item name={'amount'} label="负责人选择" >
            <InputNumber disabled={inputState}/>
          </Form.Item>
